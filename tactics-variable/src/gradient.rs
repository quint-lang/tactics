    ...
    D: Dimension,
{
    pub(crate) fn borrow(&self) -> Ref<T> {
        Ref::map(self.array.borrow(), |option| {
            option.as_ref().expect("Trying to get a de-allocated gradient. Switch on the \
            gradients first by using `._with_grad()`")
        })
    }

    pub(crate) fn borrow_mut(&self) -> RefMut<T> {
        RefMut::map(self.array.borrow_mut(), |option| {
            option.as_mut().expect("Trying to get a de-allocated gradient. Switch on the \
            gradients first by using `._with_grad()`")
        })
    }
    ...
    D: Dimension,
{
    ...
    pub(crate) fn buffer(&self) -> Ref<T> {
        Ref::map(self.buffer.borrow(), |option| {
            option.as_ref().expect("Trying to get a de-allocated gradient. Switch on the \
            gradients first by using `.with_grad()`")
        })
    }
